...
function Food(f){
    ...
    useEffect(()=>{
        if(data.strMeasure1){
            for(let i = 1; i < 21; i++){
                if(data[`strMeasure${x}`].includes("null") && !(data[`strIngredient${x}`].includes("null"))){
                    ingredients.push(
                        `${data[`strIngredient${x}`]}`
                    );
                    continue;
                } else if(data[`strIngredient${x}`].includes("null") && !(data[`strMeasure${x}`].includes("null"))){
                    ingredients.push(
                        `${data[`strMeasure${x}`]}`
                    );
                    continue;
                ...
                ingredients.push(
                    `${data[`strMeasure${x}`]} ${data[`strIngredient${x}`]}`
                )
            }
        }
    }, [data]);
